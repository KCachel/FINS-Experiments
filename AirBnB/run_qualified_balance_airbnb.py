...
output_file = "qualified_balance_airbnb.csv"
...
for i in range(len(datasets)):
    ...
    grp_data, score = fas.qualified_balance(pool_items, pool_scores, pool_groups, subset_items, subset_scores, subset_groups, q_val)

    LOCALITY.append(location[i])
    QUALIFIED_BALANCE.append(score)
    SINGLE.append(grp_data[0])
    SMALL.append(grp_data[1])
    PROFESSIONAL.append(grp_data[2])

    # dictionary of lists
    dict = {'LOCALITY': LOCALITY, 'QUALIFIED_BALANCE': QUALIFIED_BALANCE,
            'SINGLE': SINGLE,
