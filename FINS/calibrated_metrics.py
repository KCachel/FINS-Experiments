...
def calibrated_parity(pool_items, pool_scores, pool_groups, subset_items, subset_scores, subset_groups, lb_bin, ub_bin):
    ...
    fas.check_pool_subset_groups(pool_items, pool_scores, pool_groups, subset_items, subset_scores, subset_groups) #error handling
    ...
    if np.all(min_prop_each_bin == max_prop_each_bin):
        dp_val = 1 #totally fair since max = min in all bins
    else:
        different_bin_selection_rates_mask = min_prop_each_bin != max_prop_each_bin
        dp_val = np.min(min_prop_each_bin[different_bin_selection_rates_mask] / max_prop_each_bin[different_bin_selection_rates_mask])
    return bin_group_selectr, dp_val
...
def calibrated_balance(pool_items, pool_scores, pool_groups, subset_items, subset_scores, subset_groups, lb_bin, ub_bin):
    ...
    (array([[0.5, 0.5],
       [0. , 0. ]]), 1)
    """
    fas.check_pool_subset_groups(pool_items, pool_scores, pool_groups, subset_items, subset_scores,
    ...
    #bin_prop_ratios = min_props_each_bin/ max_props_each_bin



    if np.all(min_props_each_bin == max_props_each_bin):
        db_val = 1 #totally fair since max = min in all bins
    else:
        different_bin_selection_rates_mask = min_props_each_bin != max_props_each_bin
        db_val = np.min(min_props_each_bin[different_bin_selection_rates_mask] / max_props_each_bin[different_bin_selection_rates_mask])
    return bin_group_proportions, db_val
